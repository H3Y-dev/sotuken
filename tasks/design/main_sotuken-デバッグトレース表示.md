...
### 2026-08-31追補: フォールバック条件分岐を詳細表示

判定経路をさらに追えるように、目盛り範囲のフローを次の独立ノードへ分けた。

- `OCR数字検出` → `前処理4条件 一致判定`
- `VLM検出`
- `OCR/VLM候補 一致?` → `VLM候補位置 確認?`
- `OCR候補を採用` / `VLM候補を採用` / `手動選択へ`

OCRが安定した場合はVLMとの照合へ進み、不安定・未検出の場合はVLM候補の
位置確認へ進む。VLMを取得できなかった場合は、OCR候補があればOCR採用、
両方なければ手動選択へ分岐する。今回実際に通った経路だけを青い矢印と
黄色い条件ラベルで強調し、未通過経路は薄い線にして条件の全体像と実行結果を
同時に確認できるようにした。Canvas描画は`scale_flow_canvas.py`へ分離し、
判定モデルはGUIに依存しない`build_scale_flow_model()`としてテスト可能にした。

検証結果:

- 分岐モデルのテスト5件を追加し、unittest全78件合格
- 合成データで「OCR安定＋最大値のみVLM補完」と
  「OCR不安定＋VLM全面フォールバック」を1100×900pxで実描画
- OCR検出・VLM検出が別ノードで表示され、分岐ラベル、候補表、OCR一覧、
  詳細ログが同じ画面内に収まることを目視確認
