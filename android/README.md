...
初回起動時は「カメラの使用を許可してください」と許可ボタンが表示されます。許可後は背面カメラのプレビューと撮影ボタンに切り替わり、撮影画像はアプリ専用外部領域の `Pictures/captures/` に保存されます。現時点では保存画像をキューへ入れる結線は未実装なので、撮影してもキュー件数は増えません。
